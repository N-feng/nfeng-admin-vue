      ...
      <a-menu theme="light" :selectedKeys="current" mode="horizontal" @click="handleClick">
        <template v-for="item in menus">
          <a-sub-menu :key="item.path" v-if="item.children">
            <span slot="title" class="submenu-title-wrapper">{{item.meta.title}}</span>
            <template v-for="item2 in item.children">
              <a-menu-item :key="item2.name">{{item2.meta.title}}</a-menu-item>
            </template>
          </a-sub-menu>
          <a-menu-item :key="item.name" v-else>{{item.meta.title}}</a-menu-item>
        </template>
      </a-menu>
      <a-dropdown>
        <a class="ant-dropdown-link" href="javascript:;">Welcome, {{ username }}
            <a-icon type="down" />
          </a>
        <a-menu slot="overlay">
          <a-menu-item>
        ...
        .forEach((item) => {
          if (item.children) {
            item.children.forEach((item2) => {
              if (
                this.$store.getters.globalList.menus.some(
